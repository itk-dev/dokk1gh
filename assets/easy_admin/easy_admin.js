...
window.addEventListener("load", () => {
  ...
  if (document.body.classList.contains("ea-new-Code")) {
    ...
    document
      .querySelector('form[id="new-Code-form"]')
    ...
    const updateTimerange = () => {
      ...
      document.getElementById("Code_startTime").valueAsNumber =
        startTime.getTime();
    ...
    [
      ...
      document.getElementById(id)?.addEventListener("change", updateTimerange),
    );
    updateTimerange();
  }
  ...
  document.querySelectorAll(".action-expire-app").forEach((actionElement) => {
    actionElement.addEventListener("click", (event) => {
      ...
      document
        .querySelector("#modal-expire-app-button")
  ...
  if (
    document.body.classList.contains("ea-new-Guest") ||
    document.body.classList.contains("ea-edit-Guest")
  ) {
    const defaultValues = (() => {
      const el = document.querySelector(
        '[name="Guest[timeRanges][default_values]"]',
      );
      try {
    ...
    document
      .querySelectorAll('#time-ranges [type="checkbox"]')
      .forEach((el, index) => {
        const controls = el.closest("tr").querySelectorAll("select");
